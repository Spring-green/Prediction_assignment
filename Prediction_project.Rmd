...
```
```{r high_corr, include=FALSE}
M<- abs(cor(training[,-53]))
diag(M)<-0
which(M>0.8, arr.ind = T)
CorName<- findCorrelation(M,cutoff = 0.8, names = FALSE)
names(training)[CorName]
########################################################################
#preProc<- preProcess(log10(training[,-53]+1),method = "pca",pcaComp=13)
#trainPC<- predict(preProc,log10(training[,-53]+1))
#modelFit<-train(training$classe~., mehod="glm", data=trainPC)
########################################################################
```
## Building Model
